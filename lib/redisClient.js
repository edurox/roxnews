...
// Busca as notícias não-vistas dentro de uma janela do ZSET (tamanho
// proporcional a `limite`, mas devolve TODAS as não-vistas da janela — quem
// chama é quem sabe filtrar por fonte bloqueada/idioma/tag e decidir quantas
// realmente aproveitar; ver comentário dentro da função sobre por que não dá
// pra truncar aqui embaixo).
//
...
export async function buscarNaoVistas({ usuarioId, limite = 30, offset = 0 }) {
  ...
  if (!idsNaoVistos.length) {
    return { noticias: [], proximoOffset, esgotado }
  }

  // Carrega TODAS as não-vistas da janela examinada, não só as primeiras
  // `limite`. `limite` aqui só controlava quantas seriam devolvidas — mas
  // quem chama (api/feed.js) ainda precisa filtrar por fonte bloqueada,
  // idioma e tag antes de saber quais das não-vistas realmente contam. Se a
  // gente já corta pra `limite` aqui, embaixo, quem chama só enxerga essa
  // fatia pequena (normalmente as poucas mais recentes de QUALQUER assunto)
  // e nunca chega a testar contra a tag o resto da janela — foi isso que
  // fazia uma tag menos comum (ex: "tech") parecer ter só um punhado de
  // notícia mesmo com dezenas cadastradas mais pra trás na mesma janela.
  const pipeline = redis.pipeline()
  idsNaoVistos.forEach((id) => pipeline.hgetall(`noticia:${id}`))
  const resultados = await pipeline.exec()

  const noticias = resultados
  .map((noticia, i) => (noticia ? { id: idsNaoVistos[i], ...noticia } : null))
  .filter(Boolean)

  return { noticias, proximoOffset, esgotado }
}

// Rebusca notícias específicas pelo id — usado depois de resolverThumbsAoVivo
// pra pegar o imagemUrl que acabou de ser gravado (o objeto que a gente já
// tinha em mãos antes de resolver é só o snapshot de antes, sem imagem).
export async function buscarNoticiasPorIds(ids) {
  if (!ids.length) return []
    const pipeline = redis.pipeline()
    ids.forEach((id) => pipeline.hgetall(`noticia:${id}`))
    const resultados = await pipeline.exec()
    return resultados.map((noticia, i) => (noticia ? { id: ids[i], ...noticia } : null))
}
...
// ---------------------------------------------------------------------------
// Fase 2 — fila de notícias sem thumb depois da checagem rápida do RSS.
// og:image e a busca por heurística/IA rodam em background a partir daqui
// (ver lib/processarFilaThumbs.js), não antes de enfileirar.
// ---------------------------------------------------------------------------
