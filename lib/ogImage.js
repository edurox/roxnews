...
// og:image e twitter:image quase sempre estão no <head>, mas sites com
// bastante CSS/JS crítico inline antes do fechamento do <head> (comum em
// apps Rails/Next com "critical CSS") podem empurrar a meta tag bem mais
// longe do que os primeiros KB. 60KB se mostrou curto demais em alguns
// casos reais — 200KB dá bem mais margem sem gastar tempo/banda com uma
// página inteira.
const BYTES_MAXIMOS_LIDOS = 200_000
...
async function buscarInicioDoHtml(url, signal) {
  const resp = await fetch(url, {
    ...
    redirect: 'follow',
    headers: {
      // UA de navegador comum, não de bot — og:image scraping é o mesmo tipo
      // de acesso que Slack/Twitter/WhatsApp fazem pra gerar preview de link,
      // mas alguns sites bloqueiam qualquer UA que se identifique como bot
      // por nome, então evita chamar atenção à toa
  ...
  try {
    ...
    if (html) {
      ...
      if (normalizada && (await validarImagem(normalizada))) {
        ...
        console.log(`[roxnews] og:image achada para ${link}`)
      } else {
        console.log(`[roxnews] sem og:image válida para ${link}`)
      }
    } else {
      console.log(`[roxnews] html vazio/não-texto ao buscar og:image de ${link}`)
    }
  } catch (err) {
